...
// be executed against any type that implements graph.Graph.(Data Access Layers)
//The SuiteBase test depends on the gocheck framework,
...
// TestInsertLink verifies the link insert logic.
func (s *SuiteBase) TestInsertLink(c *gc.C) {
	// Create a new link
	original := &graph.Link{
...
/*
 These tests ensure that no data races occur when the code concurrently accesses the graph's contents via multiple
 iterator instances.
*/
func (s *SuiteBase) TestConcurrentLinkIterators(c *gc.C) {
	...
	for i := 0; i < numLinks; i++ {
		...
		c.Assert(s.g.InsertLink(link), gc.IsNil)
	}
...
/*
These tests verify that if we split our graph into N partitions and assign an iterator to each partition,
each iterator will receive a unique set of links/edges (that is, no item will be listed in more than one
partition) and that all the iterators will process the full set of links/edges present
in the graph.
*/
func (s *SuiteBase) TestPartitionedLinkIterators(c *gc.C) {
	...
	numPartitions := 10
	for i := 0; i < numLinks; i++ {
		c.Assert(s.g.InsertLink(&graph.Link{URL: fmt.Sprint(i)}), gc.IsNil)
	}
...
// TestInsertEdge verifies the edge upsert logic.
func (s *SuiteBase) TestInsertEdge(c *gc.C) {
	// Create links
	linkUUIDs := make([]uuid.UUID, 3)
	for i := 0; i < 3; i++ {
		link := &graph.Link{URL: fmt.Sprint(i)}
...
func (s *SuiteBase) TestConcurrentEdgeIterators(c *gc.C) {
	...
	for i := 0; i < numEdges; i++ {
		c.Assert(s.g.InsertEdge(&graph.Edge{
			Src: linkUUIDs[0],
			Dst: linkUUIDs[i],
		}), gc.IsNil)
	}
...
func (s *SuiteBase) TestEdgeIteratorTimeFilter(c *gc.C) {
	...
	for i := 0; i < len(linkUUIDs); i++ {
		...
		c.Assert(s.g.InsertEdge(edge), gc.IsNil)
		edgeUUIDs[i] = edge.ID
...
func (s *SuiteBase) TestRemoveStaleEdges(c *gc.C) {
	...
	for i := 0; i < numEdges; i++ {
		...
		c.Assert(s.g.InsertEdge(e1), gc.IsNil)
	...
	for i := 0; i < numEdges; i++ {
		...
		c.Assert(s.g.InsertEdge(e2), gc.IsNil)
	}
