...
func (s *GraphUpdaterTestSuite) TestGraphUpdater(c *gc.C) {
	...
	exp.InsertLink(linkMatcher{id: payload.LinkID, url: payload.URL, notBefore: time.Now()}).Return(nil)

	id0, id1, id2 := uuid.New(), uuid.New(), uuid.New()
	exp.InsertLink(linkMatcher{url: "http://forum.com", notBefore: time.Time{}}).DoAndReturn(setLinkID(id0))
	exp.InsertLink(linkMatcher{url: "http://example.com/foo", notBefore: time.Time{}}).DoAndReturn(setLinkID(id1))
	exp.InsertLink(linkMatcher{url: "http://example.com/bar", notBefore: time.Time{}}).DoAndReturn(setLinkID(id2))

	// We then expect two edges to be created from the origin link to the
	// two links we just created.
	exp.InsertLink(edgeMatcher{src: payload.LinkID, dst: id1}).Return(nil)
	exp.InsertLink(edgeMatcher{src: payload.LinkID, dst: id2}).Return(nil)
